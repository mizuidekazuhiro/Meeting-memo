  ...
  try {
    response = await fetch(`${OPENAI_API}/responses`, {
      ...
      body: JSON.stringify({ model, reasoning: { effort: reasoningEffort }, input: [{ role: 'system', content: [{ type: 'input_text', text: [
            ...
            'myTasks は user-action-only（ユーザー本人/当社側の実務アクションのみ）です。',
            '面談メモ完成のための編集タスク（文字起こし確認、誤変換確認、固有名詞確認、Notion追記、議事録体裁調整、参考リンク追加、要約確認など）は myTasks に絶対に入れないでください。',
            '情報が不確実という理由だけでタスクを作らないでください。メモ品質改善のためだけの項目は ambiguities に入れてください。',
            '担当者/所有者が不明な項目は myTasks に入れず ambiguities に入れてください。',
            'myTasks の各項目は具体的な目的語と業務目的を含む動詞句にしてください（「確認する」「検討する」だけの抽象表現は禁止）。',
            'otherTasks は相手側タスク、または所有者がユーザー以外/不明のアクションのみを入れてください。',
        ...
        input: [
          {
            ...
            content: [{
              ...
              text: [
                ...
                'myTasks は user-action-only（ユーザー本人/当社側の実務アクションのみ）です。面談メモ完成のための編集タスクは myTasks に絶対に入れないでください。',
                '文字起こし確認、誤変換確認、固有名詞確認、Notion本文補足、議事録体裁調整、参考リンク追加、要約再確認などメモ品質向上だけの作業は ambiguities に入れ、myTasks / otherTasks に入れないでください。',
                '担当者不明・所有者不明の項目は myTasks に入れないでください。ユーザー以外の担当は otherTasks に入れてください。',
